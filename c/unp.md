...
如何在不同的进程间交换信息呢？如果你只了解通过文件来传递信息，那么你一定需要了解一下操作系统为IPC 代表**进程间通信 (Inter Process communication)**。传统上，该术语描述了在某些操作系统上运行的不同进程之间**传递消息**的不同方式。本文还介绍了许多**同步**形式，因为较新的通信形式（例如共享内存）需要某种形式的同步才能运行。
