...
OSPF 将自治系统划分成不同的区域。区域是从逻辑上将路由器划分为不同的组，每个组用区域号（Area ID）来标识。**区域的边界是路由器**，而不是链路。一个网段（链路）只能属于一个区域，或者说每个运行 OSPF 的接口必须指明属于哪一个区域。

### 普通区域

OSPF 划分区域之后，并非所有的区域都是平等的关系。默认情况下，所有区域均属于普通区域，其中：

- 骨干区域：是中央区域，负责区域之间的路由，使用区域号 0 标识。
- 非骨干区域/标准区域：必须通过骨干区域来转发路由，因此所有非骨干区域必须与骨干区域保持连通。

### 特殊区域

OSPF 为了进一步精细化管理 LSA 的传播，还划分了 4 个特殊区域：
