...
配置Area1为Stub区域以后，R1上自治系统外部路由消失了，取而代之的是一条ABR（R2）自动下发的缺省路由，是Type3 LSA描述的。
，此时Type5 LSA（External）、Type4 LSA（Sum-Asbr）已经消失，取而代之的是一条ABR（R2）自动下发的缺省路由Type3 LSA。此时R1上仅有Type1、Type2、Type3以及Type3缺省路由这几种LSA。
...
此时R1的LSDB里面Type5 LSA（External）、Type4 LSA（Sum-Asbr）、以及描述区域间路由的Type3 LSA都已经消失，取而代之的是一条ABR（R2）自动发的缺省路由Type3 LSA。
...
## 参考
<https://forum.huawei.com/enterprise/zh/thread/580889495526588417>
